...
func NewProcessor(router *message.Router, sender Sender, sub message.Subscriber, watermillLogger watermill.LoggerAdapter) *cqrs.CommandProcessor {
	...
	if err != nil {
		panic(fmt.Errorf("creating new command processor: %w", err))
	}

	err = commandProcessor.AddHandlers(
		cqrs.NewCommandHandler("send_notification", func(ctx context.Context, cmd *SendNotification) error {
	...
	if err != nil {
		panic(fmt.Errorf("adding command handlers: %w", err))
	}
